...
The dataset consists out of 2 files: dataset/train_data.csv and dataset/test_data.csv. Each CSV contains two types of information: radar-derived data and observation-related data. Both files include the following radar-related columns:
...
Then, dataset/train_data.csv also includes observation-based columns used to validate the radar tracks. These are privileged columns, which are only available during training-time. The target labels for bird group classification are in the column bird_group . The following columns are included:
    ...
    observer_position — UPDATED: Most recent bird position in the radar track when the observer clicked the button to enter the species label to the track, encoded in EWKB. The location of the observer encoded in EWKB (Extended Well-Known Binary) Hex as Longitude / Latitude / Altitude
...
Finally, a dataset/sample_submission.csv is provided. The sample submission has the following format:
...
dataset/sample_submission.csv(84.83 kB)
